...
echo "===================================================="
echo "[Fault adaptation resume queue]"
echo "root          : $ROOT"
...
run_job "03b_fault_adapt_beta01" run_fault_adapt "0.1"
CKPT_BETA01="$(latest_ckpt_dir "$ROOT/fault_adapt_beta01")"
if [[ -n "${CKPT_BETA01:-}" && -d "$CKPT_BETA01" ]]; then
  run_job "04b_eval_fault_adapt_beta01" run_tester_eval "eval_fault_adapt_beta01" "$CKPT_BETA01"
else
  record_status "04b_eval_fault_adapt_beta01" "SKIPPED:no_ckpt"
fi

run_job "05b_fault_adapt_beta02" run_fault_adapt "0.2"
CKPT_BETA02="$(latest_ckpt_dir "$ROOT/fault_adapt_beta02")"
if [[ -n "${CKPT_BETA02:-}" && -d "$CKPT_BETA02" ]]; then
  run_job "06b_eval_fault_adapt_beta02" run_tester_eval "eval_fault_adapt_beta02" "$CKPT_BETA02"
else
  record_status "06b_eval_fault_adapt_beta02" "SKIPPED:no_ckpt"
fi
