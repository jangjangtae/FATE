...
# Focused overnight follow-up after the first pilot. Seeds 1 and 2 complement
# the pilot's seed 0 and test two lower semantic-coverage coefficients.
...
ROOT="${ROOT:-$HOME/logdir/craftax_contextual_followup_$(date +%Y%m%d_%H%M%S)}"

export PROJECT_ROOT TRAIN_ROOT ROOT
export SEEDS="${SEEDS:-1 2}"
export VARIANTS="${VARIANTS:-taskonly contextual_excess_delta_beta02 contextual_coverage_lam002 contextual_coverage_lam005}"
