...
# Isolated multi-seed queue for the contextual novelty constraint decision.
#
...
# Keep XLA's relative libdevice fallback stable for terminal and systemd runs.
cd "$PROJECT_ROOT"
...
split_steps() {
  local split="$1"
  case "$split" in
    sparse) echo "$SPARSE_EVAL_STEPS" ;;
    *) echo "$ADAPT_EVAL_STEPS" ;;
  esac
}
...
run_eval() {
  ...
  local outdir="$ROOT/seed_${seed}/$name"
...
run_variant_evals() {
  ...
  for split in $EVAL_SPLITS; do
    ...
    steps="$(split_steps "$split")"
    run_job "$seed" "eval_${variant}_${split}" \
...
for seed in $SEEDS; do
  ...
  for variant in $VARIANTS; do
    run_job "$seed" "train_${variant}" \
      run_train_variant "$seed" "$variant" "$STATS_FILE"
    ...
    run_variant_evals "$seed" "$variant" "$variant_ckpt" "$STATS_FILE"
  done
done
